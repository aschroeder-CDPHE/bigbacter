...
def db_taxa_clusters ( taxa , timestamp ) {
    // determine path to taxa database
    def clusters_path = file(params.db).resolve(taxa).resolve("clusters")

    // Build the complete database-info file in memory because GCS objects
    // cannot be opened in APPEND mode.
    def db_info_text = new StringBuilder()

    if ( clusters_path.exists() ) {
        def clusters = clusters_path.list() ?: []
        for ( cluster in clusters ) {
            def snippy_path = clusters_path.resolve(cluster).resolve("snippy")
            if ( !snippy_path.exists() ) {
                continue
            }

            def isolates = snippy_path.list() ?: []
            for ( iso in isolates ) {
                db_info_text.append(
                    taxa+"\t"+cluster+"\t"+iso.replace(".tar.gz", "")+"\n"
                )
            }
        }
    }

    // Write once to the cloud object instead of appending row by row.
    def taxadir = file(params.outdir).resolve(timestamp.toString()).resolve(taxa)
    taxadir.mkdirs()
    def db_info_file = taxadir.resolve(taxa+"-db-info.txt")
    if ( db_info_file.exists() ) {
        db_info_file.delete()
    }
    db_info_file.text = db_info_text.toString()
